...
async function execOpenAllot(itemId){
  ...
  var itemRes = WA_PLANNED.filter(function(r){return r.boq_item_id===itemId;});
  // Only show resources with remaining balance
  ...
  // Wire checkboxes to show/hide fields + wire qty/rate for est amount
  setTimeout(function(){
    document.querySelectorAll('.wa-res-chk').forEach(function(chk){
      chk.addEventListener('change',function(){
        var rid=this.getAttribute('data-res-id');
        var fields=document.querySelector('.wa-res-fields[data-res-id="'+rid+'"], .wa-res-row[data-res-id="'+rid+'"] .wa-res-fields');
        // find parent row
        var row=this.closest('.wa-res-row');
        if(row){ var f=row.querySelector('.wa-res-fields'); if(f) f.style.display=this.checked?'block':'none'; }
        // wire type change
        if(this.checked){
          var typesel=row&&row.querySelector('.wa-type-sel');
          if(typesel&&!typesel._wired){
            typesel._wired=true;
            typesel.addEventListener('change',function(){waTypeChangeFor(this.getAttribute('data-res-id'),this.value);});
          }
          var qInp=row&&row.querySelector('.wa-qty-inp');
          var rInp=row&&row.querySelector('.wa-rate-inp');
          var amtEl=row&&row.querySelector('.wa-est-amt');
          function updAmt(){if(qInp&&rInp&&amtEl)amtEl.textContent='\u20b9'+Math.round((parseFloat(qInp.value)||0)*(parseFloat(rInp.value)||0)).toLocaleString('en-IN');}
