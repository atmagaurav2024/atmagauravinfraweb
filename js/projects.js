...
function boqRender(){
  ...
  var projOpts='<option value="">-- Select Project --</option>'+
  ...
  var html=
    '<div style="margin-bottom:12px;">'+
      ...
      '<select id="boq-proj-picker" class="fsel" style="margin-bottom:0;">'+projOpts+'</select>'+
    '</div>'+
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">'+
      '<div>'+
        '<div style="font-size:16px;font-weight:800;">Bill of Quantities</div>'+
        '<div style="font-size:12px;color:var(--text2);">'+(proj?proj.name+' &middot; ':'')+' Total: '+fmtINR(total)+'</div>'+
      '</div>'+
      '<button class="btn btn-navy" onclick="boqOpenAddItem()">+ Add Item</button>'+
    '</div>';
  ...
  // Wire project picker via addEventListener — avoids any quote nesting issue
  var picker=document.getElementById('boq-proj-picker');
  if(picker){
...
function jmRender(){
  var cont=document.getElementById('proj-mod-content');if(!cont)return;
  var html='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">'+
    '<div style="font-size:16px;font-weight:800;">Joint Measurements</div>'+
    '<button class="btn btn-navy" onclick="jmOpenAdd()">+ Add JM</button></div>';
  if(!JM_JMS.length){html+='<div style="text-align:center;padding:40px;color:var(--text3);">No joint measurements found.</div>';cont.innerHTML=html;return;}
...
function planRender(){
  var cont=document.getElementById('proj-mod-content');if(!cont)return;
  var html='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">'+
    '<div style="font-size:16px;font-weight:800;">Project Planning</div>'+
    '<button class="btn btn-navy" onclick="planAddSub()">+ Add Activity</button></div>';
  if(!PLAN_ITEMS.length){html+='<div style="text-align:center;padding:40px;color:var(--text3);">No activities planned.</div>';cont.innerHTML=html;return;}
