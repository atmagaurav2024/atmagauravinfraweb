...
function projModLoadTab(){
  ...
  var configs = {
    ...
    execution:{cont:'exec-content', sel:'exec-proj-sel',  fn: function(){ WA_SUBTAB='allot'; execLoadItems(); }},
    allotted: {cont:'exec-content', sel:'exec-proj-sel',  fn: function(){ WA_SUBTAB='allotted'; execLoadItems(); }},
    daily:    {cont:'exec-content', sel:'exec-proj-sel',  fn: function(){ WA_SUBTAB='daily'; execLoadItems(); }},
    bills:    {cont:'exec-content', sel:'exec-proj-sel',  fn: function(){ WA_SUBTAB='bills'; execLoadItems(); }},
    orders:   {cont:'exec-content', sel:'exec-proj-sel',  fn: function(){ WA_SUBTAB='orders'; execLoadItems(); }}
  };
...
var WA_DAILY=[],WA_BILLS=[],WA_PAYMENTS=[],WA_ORDERS=[];
var WA_SUBTAB='allot'; // allot | allotted | daily | bills

function initExecution(){execLoadProjs();}
async function execLoadProjs(){var sel=document.getElementById('exec-proj-sel');if(!sel)return;try{var d=await sbFetch('projects',{select:'id,name',order:'name.asc'});sel.innerHTML='<option value="">— Select Project —</option>'+(Array.isArray(d)?d:[]).map(function(p){return '<option value="'+p.id+'">'+p.name+'</option>';}).join('');}catch(e){}}
async function execLoadItems(){
  ...
  }catch(e){WA_ITEMS=[];console.error(e);}
  execRenderSubTab();
}
...
function execRender(){
  ...
  // Wire allot buttons via event delegation
  setTimeout(function(){
    el.addEventListener('click',function(e){
      var btn=e.target.closest('.wa-allot-btn');
      if(btn) execOpenAllot(btn.getAttribute('data-res-id'),btn.getAttribute('data-plan-qty'),btn.getAttribute('data-unit'));
  ...
  var itemsHtml = itemsWithRes.map(function(item){
    ...
    return '<div style="background:white;border-radius:14px;border:1px solid var(--border);margin-bottom:10px;overflow:hidden;">'+
      '<div style="padding:10px 14px;background:#FFF3E0;"><span style="font-size:10px;font-family:monospace;background:#FFE0B2;color:#E65100;padding:2px 7px;border-radius:4px;">'+item.item_code+'</span><span style="font-size:13px;font-weight:800;margin-left:8px;">'+(item.short_name||item.description)+'</span></div>'+
  ...
  var topBar='<div style="display:flex;gap:8px;margin-bottom:10px;align-items:center;">'+
    '<div style="flex:1;font-size:11px;color:var(--text3);">&#9745; Check to include in combined document</div>'+
    '<button onclick="execGenCombinedDoc(\'wo\')" style="background:#E65100;color:white;border:none;border-radius:7px;padding:5px 12px;font-size:11px;font-weight:800;cursor:pointer;">&#128203; WO</button>'+
    '<button onclick="execGenCombinedDoc(\'po\')" style="background:#1565C0;color:white;border:none;border-radius:7px;padding:5px 12px;font-size:11px;font-weight:800;cursor:pointer;">&#128203; PO</button></div>';

  el.innerHTML=topBar+itemsHtml+orphanHtml;
      ...
      '<div style="border:1px solid var(--border);border-radius:10px;margin-bottom:6px;overflow:hidden;">'+
        '<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:#FAFAFA;">'+
          '<div style="flex:1;">'+
            ...
            '</div>'+
          '</div>'+
          (bal>0?'<button data-res-id="'+res.id+'" data-plan-qty="'+res.qty+'" data-unit="'+resUnit+'" class="wa-allot-btn" style="background:#E65100;color:white;border:none;border-radius:6px;padding:4px 10px;font-size:10px;font-weight:800;cursor:pointer;">+ Allot</button>':          '<span style="font-size:10px;background:#E8F5E9;color:#2E7D32;padding:3px 8px;border-radius:5px;font-weight:700;">Done</span>')+
        '</div>'+
        (rAllot.length?
          '<div style="padding:4px 12px 8px;">'+
            rAllot.map(function(a){
              ...
              return '<div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid #F5F5F5;">'+
                '<input type="checkbox" class="wa-sel-chk" data-allot-id="'+a.id+'" style="width:14px;height:14px;accent-color:#E65100;flex-shrink:0;">'+'<span style="font-size:9px;font-weight:800;padding:1px 5px;border-radius:3px;background:'+col+'20;color:'+col+';">'+lbl+'</span>'+
                ...
                '<button data-wo-id="'+a.id+'" title="Work Order" style="background:#FFF3E0;border:none;border-radius:4px;padding:2px 6px;font-size:9px;font-weight:800;cursor:pointer;color:#E65100;">WO</button>'+
                '<button data-po-id="'+a.id+'" title="Purchase Order" style="background:#E3F2FD;border:none;border-radius:4px;padding:2px 6px;font-size:9px;font-weight:800;cursor:pointer;color:#1565C0;">PO</button>'+
...
async function execOpenAllot(resId, planQty, unit){
  await loadUomIfNeeded();
  var uomOpts=buildUomOpts(unit);
  var projId=(document.getElementById('exec-proj-sel')||{}).value||'';
  var planRes=WA_PLANNED.find(function(r){return r.id===resId;})||{};
  var planRate=planRes.rate||0;

  document.getElementById('exec-sheet-title').textContent='Allot Work';
  document.getElementById('exec-sheet-body').innerHTML=
    '<div style="background:#FFF3E0;border-radius:10px;padding:8px 12px;margin-bottom:12px;font-size:11px;">'+
      '<div style="font-weight:800;color:#E65100;margin-bottom:2px;">Planned: '+planQty+' '+unit+(planRate?' @ \u20b9'+planRate:'')+'</div>'+
      '<div style="color:var(--text3);">Allotment rate may differ from planning rate</div>'+
    '</div>'+
    '<label class="flbl">Party Type *</label>'+
    '<select id="wa-type" class="fsel" onchange="waTypeChange()">'+
    ...
    '</select>'+
    '<label class="flbl" style="margin-top:8px;">Party Name *</label>'+
    '<select id="wa-party-sel" class="fsel"><option value="">\u2014 Select party type first \u2014</option></select>'+
    '<div class="g2" style="margin-top:8px;">'+
      '<div><label class="flbl">Qty *</label><input id="wa-qty" class="finp" type="number" step="0.001" max="'+planQty+'" placeholder="'+planQty+'"></div>'+
      '<div><label class="flbl">Unit</label><select id="wa-unit-sel" class="fsel">'+uomOpts+'</select></div>'+
    '</div>'+
    '<div class="g2">'+
      '<div><label class="flbl">Allotment Rate (\u20b9) *</label>'+
        '<input id="wa-rate" class="finp" type="number" step="0.01" placeholder="'+(planRate||0)+'" value="'+(planRate||'')+'"></div>'+
      '<div><label class="flbl">Est. Amount</label>'+
        '<div id="wa-est-amt" style="padding:10px 12px;background:#F8FAFC;border:1px solid var(--border);border-radius:10px;font-size:13px;font-weight:800;color:#1565C0;margin-top:2px;">\u20b90</div></div>'+
    '</div>'+
    '<label class="flbl">Scope / Terms</label>'+
    '<textarea id="wa-scope" class="ftxt" placeholder="Scope of work, terms, conditions..." rows="2" style="margin-bottom:8px;"></textarea>'+
    '<div class="g2">'+
      '<div><label class="flbl">Start Date</label><input id="wa-start" class="finp" type="date"></div>'+
      '<div><label class="flbl">End Date</label><input id="wa-end" class="finp" type="date"></div>'+
    '</div>'+
    '<label class="flbl">Generate Document after saving</label>'+
    '<div style="display:flex;gap:6px;margin-top:4px;">'+
      '<label style="display:flex;align-items:center;gap:6px;padding:7px 10px;border:1.5px solid var(--border);border-radius:8px;cursor:pointer;flex:1;">'+
        '<input type="radio" name="wa-doc" value="wo" style="accent-color:#E65100;"><div><div style="font-size:11px;font-weight:800;">Work Order</div><div style="font-size:9px;color:var(--text3);">SC / Labour</div></div>'+
      '</label>'+
      '<label style="display:flex;align-items:center;gap:6px;padding:7px 10px;border:1.5px solid var(--border);border-radius:8px;cursor:pointer;flex:1;">'+
        '<input type="radio" name="wa-doc" value="po" style="accent-color:#1565C0;"><div><div style="font-size:11px;font-weight:800;">Purchase Order</div><div style="font-size:9px;color:var(--text3);">Vendor / Material</div></div>'+
      '</label>'+
      '<label style="display:flex;align-items:center;gap:6px;padding:7px 10px;border:1.5px solid var(--border);border-radius:8px;cursor:pointer;flex:1;">'+
        '<input type="radio" name="wa-doc" value="none" checked style="accent-color:#555;"><div><div style="font-size:11px;font-weight:800;">None</div><div style="font-size:9px;color:var(--text3);">Save only</div></div>'+
      '</label>'+
    '</div>';
  ...
  sb.onclick=function(){execSaveAllot(resId,projId);};
  sf.appendChild(cb);sf.appendChild(sb);
  openSheet('ov-exec','sh-exec');

  setTimeout(function(){
    var qInp=document.getElementById('wa-qty');
    var rInp=document.getElementById('wa-rate');
    function upd(){var q=parseFloat((qInp||{}).value)||0;var r=parseFloat((rInp||{}).value)||0;var el=document.getElementById('wa-est-amt');if(el)el.textContent='\u20b9'+Math.round(q*r).toLocaleString('en-IN');}
    if(qInp)qInp.addEventListener('input',upd);
    if(rInp)rInp.addEventListener('input',upd);
    upd();
  },100);
}
...
async function execSaveAllot(resId,projId){
  var type=gv('wa-type');
  var partySel=document.getElementById('wa-party-sel');
  var party=partySel?partySel.value.trim():'';
  var qty=parseFloat(gv('wa-qty'))||0;
  if(!type||!party){toast('Party type and name required','warning');return;}
  if(!qty){toast('Qty required','warning');return;}
  var rate=parseFloat(gv('wa-rate'))||0;
  if(!rate){toast('Allotment rate required','warning');return;}
  var unitSel=document.getElementById('wa-unit-sel');
  var unit=(unitSel&&unitSel.value)?unitSel.value:null;
  var planRes=WA_PLANNED.find(function(r){return r.id===resId;})||{};
  var docRadio=document.querySelector('input[name="wa-doc"]:checked');
  var docType=docRadio?docRadio.value:'none';

  try{
    var res=await sbInsert('boq_exec_resources',{
      project_id:projId,
      boq_item_id:planRes.boq_item_id||null,
      boq_subitem_id:planRes.boq_subitem_id||null,
      boq_exec_resource_id:resId,
      date:new Date().toISOString().slice(0,10),
      exec_type:type,
      party_name:party,qty:qty,unit:unit,rate:rate,
      scope:gv('wa-scope')||null,
      start_date:gv('wa-start')||null,
      end_date:gv('wa-end')||null
    });
    if(res&&res[0]) WA_ALLOT.push(res[0]);
    closeSheet('ov-exec','sh-exec');
    toast(party+' allotted!','success');
    execRender();

    // Generate WO or PO if requested
    if(docType==='wo' && res&&res[0]){
      setTimeout(function(){ generateWorkOrder(res[0]); }, 400);
    } else if(docType==='po' && res&&res[0]){
      setTimeout(function(){ generatePurchaseOrder(res[0]); }, 400);
    }
  }catch(e){toast('Error: '+e.message,'error');console.error(e);}
}
...
function execRenderAllotted(){
  ...
  WA_ALLOT.forEach(function(a){
    ...
    if(!parties[key]) parties[key]={type:a.exec_type,name:a.party_name,items:[]};
    parties[key].items.push(a);
  });
  var tLbl={vendor:'Vendor',sc:'SC',labour_contractor:'Labour Contr.',labour:'Labour',machinery:'Machinery'};
  var tCol={vendor:'#1565C0',sc:'#6A1B9A',labour_contractor:'#2E7D32',labour:'#37474F',machinery:'#E65100'};

  el.innerHTML=Object.keys(parties).map(function(key){
    var p=parties[key];var col=tCol[p.type]||'#37474F';
    // Existing orders for this party
    var pOrders=WA_ORDERS.filter(function(o){return o.party_name===p.name&&o.party_type===p.type;});
    var totalOrdered=pOrders.reduce(function(s,o){return s+(parseFloat(o.qty)||0);},0);
    var totalAmt=p.items.reduce(function(s,a){return s+Math.round((parseFloat(a.qty)||0)*(parseFloat(a.rate)||0));},0);
    var totalQty=p.items.reduce(function(s,a){return s+(parseFloat(a.qty)||0);},0);

    var itemRows=p.items.map(function(a){
      // Check if this item has an order
      var itemOrders=WA_ORDERS.filter(function(o){return o.allot_id===a.id;});
      var orderedQty=itemOrders.reduce(function(s,o){return s+(parseFloat(o.qty)||0);},0);
      var balQty=Math.max(0,(parseFloat(a.qty)||0)-orderedQty);
      var hasFullOrder=orderedQty>=(parseFloat(a.qty)||0);
      return '<div style="display:flex;align-items:center;gap:8px;padding:7px 12px;border-bottom:1px solid #F5F5F5;">'+
        '<input type="checkbox" class="allot-chk" data-allot-id="'+a.id+'" data-qty="'+a.qty+'" data-rate="'+(a.rate||0)+'" data-unit="'+(a.unit||'')+'" data-bal="'+balQty+'"'+(hasFullOrder?' disabled':'')+' style="width:15px;height:15px;accent-color:'+col+';flex-shrink:0;">'+
        '<div style="flex:1;min-width:0;">'+
          '<div style="font-size:12px;font-weight:700;">'+(a.party_name||'Resource')+'</div>'+
          '<div style="font-size:10px;color:var(--text3);">'+(a.unit?a.qty+' '+a.unit:'Qty: '+a.qty)+(a.rate?' @ &#8377;'+a.rate:'')+(a.scope?'<br>'+a.scope:'')+'</div>'+
        '</div>'+
        '<div style="text-align:right;flex-shrink:0;">'+
          '<div style="font-size:11px;font-weight:800;">&#8377;'+Math.round((parseFloat(a.qty)||0)*(parseFloat(a.rate)||0)).toLocaleString('en-IN')+'</div>'+
          (orderedQty>0?'<div style="font-size:9px;color:#2E7D32;font-weight:700;">Ordered: '+orderedQty+(parseFloat(a.qty)>orderedQty?' | Bal: '+balQty.toFixed(2):'&#10003;')+'</div>':
           '<div style="font-size:9px;color:var(--text3);">Not ordered</div>')+
        '</div>'+
        (itemOrders.length?itemOrders.map(function(o){
          return '<div style="font-size:9px;background:#E8F5E9;color:#2E7D32;padding:1px 6px;border-radius:4px;font-weight:700;cursor:pointer;" onclick="execViewOrder(\''+o.id+'\')">'+o.doc_type.toUpperCase()+'-'+o.doc_number+'</div>';
        }).join(''):'')+
        ...
        '<button onclick="execDelAllotted(\''+a.id+'\')" title="Delete allotment" style="background:none;border:none;color:#C62828;font-size:16px;cursor:pointer;flex-shrink:0;">&#215;</button>'+
      '</div>';
    }).join('');

    return '<div style="background:white;border-radius:14px;border:1px solid var(--border);margin-bottom:12px;overflow:hidden;" id="allotted-'+key.replace(/[^a-z0-9]/gi,'-')+'">'+
      // Party header
      '<div style="padding:10px 14px;background:'+col+'10;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;">'+
        ...
        '<div style="flex:1;font-size:14px;font-weight:800;">'+p.name+'</div>'+
        '<div style="text-align:right;font-size:10px;color:var(--text3);">'+p.items.length+' items | &#8377;'+Math.round(totalAmt).toLocaleString('en-IN')+'</div>'+
      '</div>'+
      ...
      '<div style="padding:10px 14px;background:#FAFAFA;border-top:1px solid var(--border);display:flex;gap:8px;align-items:center;">'+
        '<div style="flex:1;font-size:10px;color:var(--text3);">Select items above to include in document</div>'+
        '<button onclick="execGenPartyDoc(\''+key+'\',\'wo\')" style="background:#E65100;color:white;border:none;border-radius:7px;padding:5px 12px;font-size:11px;font-weight:800;cursor:pointer;">&#128203; Work Order</button>'+
        '<button onclick="execGenPartyDoc(\''+key+'\',\'po\')" style="background:#1565C0;color:white;border:none;border-radius:7px;padding:5px 12px;font-size:11px;font-weight:800;cursor:pointer;">&#128203; Purchase Order</button>'+
      '</div>'+
    '</div>';
  }).join('');
}
