...
function projModRenderNav(){
  ...
  var mainRow='<div style="display:flex;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch;padding:6px 10px;gap:4px;background:var(--card-bg);">'+
...
async function boqDownloadExcel(){
  ...
  lines.push(['','','','','','GRAND TOTAL',Math.round(total),'']);
  ...
  a.download='BOQ_'+(projName||'Project').replace(/[^a-z0-9]/gi,'_')+'.csv';
  a.click();
...
async function boqDownloadPDF(){
  ...
  var thead='<tr style="background:#4A148C;color:white;">'+
    ...
    '<th style="'+thStyle+'text-align:left;">Short Name</th>'+
  ...
  var html='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>BOQ - '+projName+'</title>'+
    '<style>'+
...
async function execLoadItems(silent){
  ...
  try{
    ...
    WA_COMBINED_PLAN_ITEMS=pgIds.length?await sbFetch('combined_plan_items',{select:'*',filter:'group_id=in.('+pgIds.join(',')+')'}):[];
    if(!Array.isArray(WA_COMBINED_PLAN_ITEMS)) WA_COMBINED_PLAN_ITEMS=[];

    var rg=await sbFetch('combined_rr_groups',{select:'*',filter:'project_id=eq.'+projId+'&status=eq.approved',order:'created_at.desc'});
    WA_COMBINED_RR_GROUPS=Array.isArray(rg)?rg:[];
    var rgIds=WA_COMBINED_RR_GROUPS.map(function(g){return g.id;});
    WA_COMBINED_RR_ITEMS=rgIds.length?await sbFetch('combined_rr_items',{select:'*',filter:'group_id=in.('+rgIds.join(',')+')'}):[];
    if(!Array.isArray(WA_COMBINED_RR_ITEMS)) WA_COMBINED_RR_ITEMS=[];
  }catch(e){
    WA_COMBINED_PLAN_ITEMS=[]; WA_COMBINED_RR_GROUPS=[]; WA_COMBINED_RR_ITEMS=[];
    console.warn('combined planning/RR tables not available yet — run the migration', e);
  }
...
// ════════════════════════════════════════════════════════════════
var RR_ITEMS=[], RR_PLAN_ITEMS=[], RR_PLAN_SUBS=[], RR_PLAN_RES=[], RR_COMBINED_PLAN_GROUPS=[], RR_COMBINED_PLAN_ITEMS=[], RR_COMBINED_RR_GROUPS=[], RR_COMBINED_RR_ITEMS=[];
...
async function rrLoadItems(){
  ...
  var projId=PROJ_MOD_SEL_ID||'';
  if(!projId){
    el.innerHTML='<div style="text-align:center;padding:40px;color:var(--text3);background:var(--card-bg);border-radius:12px;">Select a project from the dropdown above</div>';
    return;
  }
  ...
  try{
    var pg=await sbFetch('combined_plan_groups',{select:'*',filter:'project_id=eq.'+projId,order:'created_at.desc'});
    RR_COMBINED_PLAN_GROUPS=Array.isArray(pg)?pg:[];
    var pgIds=RR_COMBINED_PLAN_GROUPS.map(function(g){return g.id;});
    RR_COMBINED_PLAN_ITEMS=pgIds.length?await sbFetch('combined_plan_items',{select:'*',filter:'group_id=in.('+pgIds.join(',')+')'}):[];
    if(!Array.isArray(RR_COMBINED_PLAN_ITEMS)) RR_COMBINED_PLAN_ITEMS=[];

    var rg=await sbFetch('combined_rr_groups',{select:'*',filter:'project_id=eq.'+projId,order:'created_at.desc'});
    RR_COMBINED_RR_GROUPS=Array.isArray(rg)?rg:[];
    var rgIds=RR_COMBINED_RR_GROUPS.map(function(g){return g.id;});
    RR_COMBINED_RR_ITEMS=rgIds.length?await sbFetch('combined_rr_items',{select:'*',filter:'group_id=in.('+rgIds.join(',')+')'}):[];
    if(!Array.isArray(RR_COMBINED_RR_ITEMS)) RR_COMBINED_RR_ITEMS=[];
  }catch(e){
    RR_COMBINED_PLAN_GROUPS=[]; RR_COMBINED_PLAN_ITEMS=[]; RR_COMBINED_RR_GROUPS=[]; RR_COMBINED_RR_ITEMS=[];
    console.warn('combined planning/RR tables not available yet — run the migration', e);
  }
  rrRender();
}
...
function rrRender(){
  ...
  var summaryBar=
    '<div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;">'+
      Object.keys(counts).filter(function(k){return counts[k]>0;}).map(function(k){
        return '<div style="background:var(--card-bg);border-radius:10px;padding:8px 14px;border-left:4px solid '+statusColors[k]+';flex:1;min-width:80px;">'+
  ...
  if(RR_COMBINED_PLAN_GROUPS.length){
    var groupCards=RR_COMBINED_PLAN_GROUPS.map(function(g){
      var gItems=RR_COMBINED_PLAN_ITEMS.filter(function(it){return it.group_id===g.id;});
      var lineRows=gItems.map(function(it){
        var raised=RR_COMBINED_RR_ITEMS.filter(function(ri){return ri.plan_item_id===it.id;})
          .reduce(function(s,ri){return s+(parseFloat(ri.qty)||0);},0);
        var remaining=Math.max(0,(parseFloat(it.qty)||0)-raised);
        var boqItem=itemById[it.boq_item_id];
        return {it:it, boqItem:boqItem, remaining:remaining};
      });
    ...
    var raisedGroupCards=RR_COMBINED_RR_GROUPS.map(function(g){
      var giItems=RR_COMBINED_RR_ITEMS.filter(function(ri){return ri.group_id===g.id;});
      var sc=statusColors[g.status]||'#555'; var sl=statusLabels[g.status]||g.status;
        ...
        '<div style="display:flex;justify-content:space-between;align-items:center;">'+
          ...
          '<div style="display:flex;gap:4px;align-items:center;">'+
            ...
            (g.status==='pending'
              ...
              : g.status==='approved'
                ? '<button onclick="rrGroupOpenAllotForm(\''+g.id+'\')" style="background:#1565C0;color:white;border:none;border-radius:5px;padding:4px 10px;font-size:10px;font-weight:700;cursor:pointer;">&#128203; Allot Group</button>'
                : '')+
    ...
    combinedSectionHtml=
      '<div style="font-size:11.5px;font-weight:800;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;">Combined Planning Groups</div>'+
      groupCards+
      (raisedGroupCards?'<div style="font-size:11.5px;font-weight:800;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin:14px 0 6px;">Combined RRs Raised</div>'+raisedGroupCards:'');
  }
  ...
  el.innerHTML=
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px;">'+
      ...
      '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">'+
        ...
        '<button onclick="rrCombinedPrompt()" style="background:#00838F;color:white;border:none;border-radius:8px;padding:6px 10px;font-size:11px;font-weight:800;cursor:pointer;">&#128203; Combined RR</button>'+
    ...
    combinedSectionHtml+
    (itemCards||'<div style="text-align:center;padding:40px;color:var(--text3);background:var(--card-bg);border-radius:12px;">No planned resources found. Add resources in Planning tab first.</div>');
}
...
// ── Combined RR: raise requisitions for multiple planned resources
// at once instead of one item at a time. ──────────────────────────
function rrCombinedPrompt(){
  var projId=PROJ_MOD_SEL_ID||'';
  if(!projId){toast('Select a project first','warning');return;}

  var eligible=[];
  RR_PLAN_ITEMS.forEach(function(item){
    var iRes=RR_PLAN_RES.filter(function(r){return r.boq_item_id===item.id;});
    iRes.forEach(function(res){
      ...
      var planQty=parseFloat(res.qty)||0;
      var remaining=Math.max(0,planQty-raised);
      if(remaining>0.0001) eligible.push({item:item, res:res, remaining:remaining});
    });
  });

  if(!eligible.length){ toast('No balance left to raise — every planned resource is already fully requisitioned','warning'); return; }

  var byParty={}; var partyOrder=[];
  eligible.forEach(function(x,idx){ x._idx=idx; var p=x.res.party_name||'—'; if(!byParty[p]){byParty[p]=[]; partyOrder.push(p);} byParty[p].push(x); });

  var listHtml=partyOrder.map(function(party){
    var rows=byParty[party].map(function(x){
      var idx=x._idx, maxQty=x.remaining;
      return '<tr><td style="padding:3px 6px;"><input type="checkbox" class="rr-cmb-item-chk" data-idx="'+idx+'" data-party="'+esc(party)+'" checked style="width:14px;height:14px;accent-color:#00838F;"></td>'+
        '<td style="padding:3px 6px;font-family:monospace;font-size:10px;">'+x.item.item_code+'</td>'+
        '<td style="padding:3px 6px;font-size:11px;">'+(x.item.short_name||x.item.description||'')+'</td>'+
        '<td style="padding:3px 6px;text-align:right;">'+
    ...
    var groupHeader='<tr><td colspan="4" style="padding:5px 6px;background:#E0F7FA;font-size:10.5px;font-weight:800;color:#00838F;">'+
      esc(party)+' <span style="font-weight:600;color:#4A9AA5;">('+byParty[party].length+' item'+(byParty[party].length!==1?'s':'')+')</span>'+
      ' <span onclick="rrCombinedToggleParty(this,\''+esc(party)+'\')" style="float:right;font-weight:700;color:#00838F;cursor:pointer;text-decoration:underline;">select only these</span>'+
      '</td></tr>';
    return groupHeader+rows;
  }).join('');

  var ov=document.getElementById('rr-combined-ov');
  if(ov) ov.remove();
  var d=document.createElement('div');
  d.id='rr-combined-ov';
  d.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;';
  d.innerHTML='<div style="background:var(--card-bg);border-radius:16px;max-width:480px;width:100%;max-height:85vh;overflow-y:auto;padding:18px;font-family:Nunito,sans-serif;">'+
    ...
    '</div></div>';
  document.body.appendChild(d);
  window._rrCombinedEligible=eligible;
}
function rrCombinedToggleAll(el){
  var toAll=el.getAttribute('data-state')!=='all';
  document.querySelectorAll('.rr-cmb-item-chk').forEach(function(chk){ chk.checked=toAll; });
  el.setAttribute('data-state', toAll?'all':'none');
  el.textContent=toAll?'Deselect all':'Select all';
}
function rrCombinedToggleParty(el, party){
  document.querySelectorAll('.rr-cmb-item-chk').forEach(function(chk){
    chk.checked = (chk.getAttribute('data-party')===party);
  });
}
async function rrCombinedConfirm(){
  var msg=document.getElementById('rr-cmb-msg');
  var go=document.getElementById('rr-cmb-go');
  var setMsg=function(t){ if(msg) msg.textContent=t; };
  var date=(document.getElementById('rr-cmb-date')||{}).value||'';
  if(!date){ setMsg('Required date is needed'); return; }
  var remarks=(document.getElementById('rr-cmb-remarks')||{}).value||'';
  var reqBy=(document.getElementById('rr-cmb-by')||{}).value||'';
  var projId=PROJ_MOD_SEL_ID||'';
  if(!projId){ setMsg('No project selected'); return; }

  var allEligible=window._rrCombinedEligible||[];
  var checkedIdx={};
  document.querySelectorAll('.rr-cmb-item-chk').forEach(function(chk){ if(chk.checked) checkedIdx[chk.getAttribute('data-idx')]=true; });
  var qtyByIdx={};
  var qtyError=null;
  document.querySelectorAll('.rr-cmb-item-qty').forEach(function(inp){
    var idx=inp.getAttribute('data-idx');
    if(!checkedIdx[idx]) return;
    var q=parseFloat(inp.value);
    var max=parseFloat(inp.getAttribute('data-max'))||0;
    if(!q||q<=0){ qtyError='Enter a valid quantity for every selected item'; }
    else if(q>max+0.0001){ qtyError='Quantity for '+(allEligible[idx]?allEligible[idx].item.item_code:'an item')+' can\'t exceed its available '+max; }
    qtyByIdx[idx]=q;
  });
  if(qtyError){ setMsg(qtyError); return; }
  var selected=allEligible.filter(function(x,idx){ return checkedIdx[idx]; }).map(function(x){
    var idx=allEligible.indexOf(x);
    return Object.assign({}, x, {raiseQty: qtyByIdx[idx]});
  });
  if(!selected.length){ setMsg('Select at least one item'); return; }

  if(go){ go.disabled=true; go.style.opacity='.6'; }
  var ok=0, failed=0;
  for(var i=0;i<selected.length;i++){
    var x=selected[i];
    setMsg('Raising… '+(i+1)+' of '+selected.length);
    ...
    try{
      var res=await sbInsert('resource_requisitions',{
        project_id:projId,
        boq_item_id:x.item.id,
        plan_res_id:x.res.id,
        party_name:x.res.party_name,
        party_type:x.res.exec_type,
        qty:x.raiseQty,
        unit:x.res.unit||null,
        required_date:date,
      ...
      if(res&&res[0]){ RR_ITEMS.push(res[0]); ok++; } else failed++;
    }catch(e){ console.error('combined RR failed for '+x.item.item_code, e); failed++; }
  }

  var ov=document.getElementById('rr-combined-ov'); if(ov) ov.remove();
  rrRender();
  if(failed) toast(ok+' RR'+(ok!==1?'s':'')+' raised, '+failed+' failed — see console','warning');
  else toast(ok+' RR'+(ok!==1?'s':'')+' raised','success');
}

// ── Raise a single merged combined RR for one specific combined-
// planning group, covering whatever balance remains on its items. ──
function rrOpenGroupPrompt(groupId){
  var projId=PROJ_MOD_SEL_ID||'';
  var group=RR_COMBINED_PLAN_GROUPS.find(function(g){return g.id===groupId;});
  if(!group){ toast('Group not found','error'); return; }
  var itemById={}; RR_PLAN_ITEMS.forEach(function(it){ itemById[it.id]=it; });
  var gItems=RR_COMBINED_PLAN_ITEMS.filter(function(it){return it.group_id===groupId;});
  var rows=gItems.map(function(it){
    var raised=RR_COMBINED_RR_ITEMS.filter(function(ri){return ri.plan_item_id===it.id;})
      .reduce(function(s,ri){return s+(parseFloat(ri.qty)||0);},0);
    var remaining=Math.max(0,(parseFloat(it.qty)||0)-raised);
    var boqItem=itemById[it.boq_item_id];
    return {planItem:it, boqItem:boqItem, remaining:remaining};
  }).filter(function(r){return r.remaining>0.0001;});

  if(!rows.length){ toast('Nothing left to raise for this group','warning'); return; }

  var listHtml=rows.map(function(r,idx){
    var maxQty=r.remaining;
  ...
  var ov=document.getElementById('rr-group-ov');
  if(ov) ov.remove();
  var d=document.createElement('div');
  d.id='rr-group-ov';
  d.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;';
...
async function rrGroupConfirm(groupId){
  var msg=document.getElementById('rr-grp-msg');
  var go=document.getElementById('rr-grp-go');
  var setMsg=function(t){ if(msg) msg.textContent=t; };
  var group=RR_COMBINED_PLAN_GROUPS.find(function(g){return g.id===groupId;});
  if(!group){ setMsg('Group not found'); return; }
  var date=(document.getElementById('rr-grp-date')||{}).value||'';
  if(!date){ setMsg('Required date is needed'); return; }
  var remarks=(document.getElementById('rr-grp-remarks')||{}).value||'';
  var reqBy=(document.getElementById('rr-grp-by')||{}).value||'';
  var projId=PROJ_MOD_SEL_ID||'';
  if(!projId){ setMsg('No project selected'); return; }

  var rows=[]; var qtyError=null;
  document.querySelectorAll('.rr-grp-item-qty').forEach(function(inp){
    var q=parseFloat(inp.value);
    var max=parseFloat(inp.getAttribute('data-max'))||0;
    if(!q||q<=0){ qtyError='Enter a valid quantity for every item'; return; }
    if(q>max+0.0001){ qtyError='Quantity can\'t exceed the available balance'; return; }
    rows.push({plan_item_id:inp.getAttribute('data-plan-item-id'), boq_item_id:inp.getAttribute('data-boq-item-id'), qty:q});
  });
  if(qtyError){ setMsg(qtyError); return; }
  if(!rows.length){ setMsg('Nothing to raise'); return; }

  if(go){ go.disabled=true; go.style.opacity='.6'; }
  setMsg('Raising…');
  var rrNo='RR/'+projId.slice(-4).toUpperCase()+'/'+new Date().getFullYear()+'/C'+
  ...
  try{
    ...
    if(!newGroupId) throw new Error('Could not create the RR group');
    var itemRows=rows.map(function(r){
      return {group_id:newGroupId, boq_item_id:r.boq_item_id, plan_item_id:r.plan_item_id,
        party_name:group.party_name, party_type:group.exec_type, qty:r.qty};
    });
    await sbInsert('combined_rr_items', itemRows);
    toast(rrNo+' raised for '+group.party_name,'success');
    var ov=document.getElementById('rr-group-ov'); if(ov) ov.remove();
    await rrLoadItems();
  }catch(e){
    setMsg('Error: '+e.message+' — has the migration been run?');
    if(go){go.disabled=false;go.style.opacity='1';}
  }
}
async function rrGroupApprove(groupId){
  try{
    await sbUpdate('combined_rr_groups', groupId, {status:'approved'});
    toast('Approved','success');
    await rrLoadItems();
  }catch(e){ toast('Error: '+e.message,'error'); }
}
async function rrGroupReject(groupId){
  try{
...
function rrGroupOpenAllotForm(groupId){
  ...
  // De-duplicate in case both RR-tab and WA-tab arrays happened to be loaded
  ...
  document.getElementById('exec-sheet-body').innerHTML=
    ...
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">'+
      ...
      '<div><label class="flbl">End Date</label><input id="rr-grp-allot-end" class="finp" type="date"></div>'+
    '</div>'+
    '<div style="margin-bottom:8px;"><label class="flbl">Scope / Terms</label>'+
      '<textarea id="rr-grp-allot-scope" class="ftxt" rows="2" placeholder="Common scope for this allotment...">'+(group.remarks||'')+'</textarea>'+
    '</div>'+
    '<div><label class="flbl">Document Type (optional)</label>'+
      '<div style="display:flex;gap:6px;margin-top:4px;">'+
        '<label style="display:flex;align-items:center;gap:5px;padding:6px 10px;border:1.5px solid var(--border);border-radius:7px;cursor:pointer;flex:1;">'+
          '<input type="radio" name="rr-grp-doc-type" value="wo" style="accent-color:#E65100;">'+
          '<div style="font-size:10px;font-weight:800;">Work Order</div>'+
        '</label>'+
        '<label style="display:flex;align-items:center;gap:5px;padding:6px 10px;border:1.5px solid var(--border);border-radius:7px;cursor:pointer;flex:1;">'+
          '<input type="radio" name="rr-grp-doc-type" value="po" style="accent-color:#1565C0;">'+
...
function execRender(){
  ...
  // Combined RR groups approved and awaiting allotment
  var itemByIdWA={}; WA_ITEMS.forEach(function(it){ itemByIdWA[it.id]=it; });
  var combinedAllotHtml='';
  if(WA_COMBINED_RR_GROUPS.length){
    var cards=WA_COMBINED_RR_GROUPS.map(function(g){
      var giItems=WA_COMBINED_RR_ITEMS.filter(function(ri){return ri.group_id===g.id;});
      var alreadyAllotted=WA_ALLOT.filter(function(a){return a.combined_rr_group_id===g.id;});
      var allottedItemIds={}; alreadyAllotted.forEach(function(a){ if(a.combined_rr_item_id) allottedItemIds[a.combined_rr_item_id]=true; });
      var remainingItems=giItems.filter(function(ri){return !allottedItemIds[ri.id];});
    ...
    if(cards){
      combinedAllotHtml='<div style="font-size:11.5px;font-weight:800;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;">Combined RRs Awaiting Allotment</div>'+cards;
    }
  }

  if(!WA_PLANNED.length){
    el.innerHTML=combinedAllotHtml||'<div style="text-align:center;padding:40px;color:var(--text3);"><div style="font-size:32px;">&#128230;</div><div style="font-weight:700;margin-top:8px;">No planned resources</div><div style="font-size:12px;margin-top:4px;">Add resources in the Planning tab first</div></div>';
    return;
  }
  ...
  el.innerHTML=dlBtns+combinedAllotHtml+multiBtn+itemsHtml+orphanHtml;
...
function execRenderOtherExpenses(){
  var el=document.getElementById('exec-content');if(!el)return;
  var tabBar='<div style="display:flex;align-items:center;gap:0;border-bottom:2px solid var(--border);margin-bottom:0;background:var(--card-bg);position:sticky;top:0;z-index:10;overflow-x:auto;">'+
...
function execRenderSales(){
  ...
  var tabBar='<div style="display:flex;align-items:center;gap:0;border-bottom:2px solid var(--border);margin-bottom:0;background:var(--card-bg);position:sticky;top:0;z-index:10;">'+
    tabs.map(function(t){
          ...
          '<table style="width:100%;border-collapse:collapse;font-size:10px;">'+
            ...
            items.map(function(x){
              return '<tr style="border-bottom:1px solid #F0F0F0;">'+
                ...
                '<td style="padding:5px 10px;text-align:right;">'+inr(x.rate)+'</td>'+
  ...
  // ── Generate Bill tab ──
  // Compute cumulative DPR qty per BOQ item
  var items=WA_ITEMS.filter(function(i){return i.project_id===projId;}).slice().sort(function(a,b){
    var ca=(a.item_code||'').split('.').map(function(n){return parseInt(n,10)||0;});
    var cb=(b.item_code||'').split('.').map(function(n){return parseInt(n,10)||0;});
    for(var i=0;i<Math.max(ca.length,cb.length);i++){var diff=(ca[i]||0)-(cb[i]||0);if(diff!==0)return diff;}
    return 0;
  ...
  var itemRows=items.map(function(item){
    ...
    var prevBilled=prevBilledByItem[item.id]||0;
    var unbilledQty=Math.max(0,doneQty-prevBilled);
    ...
    return '<tr style="border-bottom:1px solid #F0F0F0;" id="sr-'+item.id+'">'+
      '<td style="padding:8px 10px;vertical-align:middle;">'+
        '<div style="display:flex;align-items:center;gap:6px;">'+
          '<input type="checkbox" class="sl-item-chk" data-item-id="'+item.id+'" data-rate="'+rate+'" '+
            'style="width:15px;height:15px;accent-color:#7B1FA2;flex-shrink:0;" '+(unbilledQty>0?'checked':'')+' onchange="slUpdateTotal()">'+
          '<div>'+
            '<span style="font-size:9px;font-family:monospace;background:#EDE7F6;color:#7B1FA2;padding:1px 5px;border-radius:3px;margin-right:4px;">'+item.item_code+'</span>'+
            '<span style="font-size:11px;font-weight:700;">'+(item.short_name||item.description)+'</span>'+
          '</div>'+
        '</div>'+
      '</td>'+
      '<td style="padding:8px 10px;text-align:right;font-size:10px;color:var(--text3);">'+boqQty+' '+(item.unit||'')+'</td>'+
      '<td style="padding:8px 10px;text-align:right;font-size:11px;font-weight:700;color:'+pctCol+';">'+
        doneQty.toFixed(2).replace(/\.?0+$/,'')+' '+(item.unit||'')+
      ...
      '<td style="padding:8px 10px;text-align:right;">'+
        '<input type="number" class="finp sl-qty-inp" data-item-id="'+item.id+'" data-rate="'+rate+'" '+
          ...
          'style="width:80px;text-align:right;padding:3px 6px;font-size:11px;" oninput="slUpdateTotal()">'+
        '<div style="font-size:9px;color:var(--text3);margin-top:2px;">'+(item.unit||'')+'</div>'+
      '</td>'+
      '<td style="padding:8px 10px;text-align:right;font-size:11px;font-weight:800;color:#4A148C;" id="sl-amt-'+item.id+'">'+inr(amt)+'</td>'+
    '</tr>';
  }).join('');
      ...
      '<div style="background:var(--card-bg);border-radius:12px;border:1px solid var(--border);overflow:hidden;margin-bottom:10px;">'+
        '<div style="padding:10px 14px;background:#F3E5F5;display:flex;align-items:center;gap:10px;">'+
          ...
          '<div style="display:flex;gap:8px;align-items:center;">'+
            '<div>'+
              '<label style="font-size:9px;color:var(--text3);font-weight:700;display:block;">Bill Date</label>'+
              '<input type="date" id="sl-date" class="finp" value="'+new Date().toISOString().slice(0,10)+'" style="font-size:11px;padding:4px 8px;">'+
            '</div>'+
            '<div>'+
              '<label style="font-size:9px;color:var(--text3);font-weight:700;display:block;">Bill Ref</label>'+
              '<input type="text" id="sl-ref" class="finp" placeholder="SALES/'+new Date().getFullYear()+'/'+String(nextBillNo).padStart(4,'0')+'" style="font-size:11px;padding:4px 8px;width:120px;">'+
            '</div>'+
          '</div>'+
        '</div>'+
        ...
        '<div style="padding:10px 14px;border-top:1px solid var(--border);">'+
          ...
          '<div style="display:flex;gap:6px;flex-wrap:wrap;">'+
            '<button onclick="blAddAddition()" style="font-size:10px;padding:4px 10px;background:#E8F5E9;color:#2E7D32;border:1px solid #C8E6C9;border-radius:5px;cursor:pointer;font-weight:700;">+ Custom</button>'+
        ...
        '<div style="padding:10px 14px;border-top:1px solid var(--border);">'+
          ...
          '<div style="display:flex;gap:6px;">'+
            '<button onclick="blAddGst(18)" style="font-size:10px;padding:4px 10px;background:#E8F5E9;color:#1B5E20;border:1px solid #C8E6C9;border-radius:5px;cursor:pointer;font-weight:700;">+ GST 18%</button>'+
            '<button onclick="blAddGst(12)" style="font-size:10px;padding:4px 10px;background:#E8F5E9;color:#1B5E20;border:1px solid #C8E6C9;border-radius:5px;cursor:pointer;font-weight:700;">+ GST 12%</button>'+
            '<button onclick="blAddGst(5)" style="font-size:10px;padding:4px 10px;background:#E8F5E9;color:#1B5E20;border:1px solid #C8E6C9;border-radius:5px;cursor:pointer;font-weight:700;">+ GST 5%</button>'+
          '</div>'+
        '</div>'+
...
function slUpdateTotal(){
  var inr=function(n){return '\u20b9'+Number(n||0).toLocaleString('en-IN',{minimumFractionDigits:0});};
  var workTotal=0;
  document.querySelectorAll('.sl-qty-inp').forEach(function(inp){
    var itemId=inp.getAttribute('data-item-id');
    var rate=parseFloat(inp.getAttribute('data-rate'))||0;
    var chk=document.querySelector('.sl-item-chk[data-item-id="'+itemId+'"]');
    var qty=chk&&chk.checked?(parseFloat(inp.value)||0):0;
    var amt=Math.round(qty*rate);
    var amtEl=document.getElementById('sl-amt-'+itemId);
    if(amtEl) amtEl.textContent=inr(amt);
    workTotal+=amt;
  });
  var wsTot=document.getElementById('sl-work-subtotal');
  if(wsTot) wsTot.textContent=inr(workTotal);
  // Additions
  var addTotal=0;
  ...
  if(nbgEl) nbgEl.textContent=inr(Math.round(netBeforeGst));
  // GST
  var gstTotal=0;
  document.querySelectorAll('.bl-gst-row').forEach(function(row){
    ...
    if(pct&&amtInp){var calcAmt=Math.round(netBeforeGst*pct/100);amtInp.value=calcAmt;}
    gstTotal+=parseFloat(amtInp&&amtInp.value)||0;
  });
  var grossTotal=netBeforeGst+gstTotal;
  var totEl=document.getElementById('sl-total');
  if(totEl) totEl.textContent=inr(Math.round(grossTotal));
}

async function execSaveSalesBill(){
  var projId=PROJ_MOD_SEL_ID||(document.getElementById('exec-proj-sel')||{}).value||'';
  var date=(document.getElementById('sl-date')||{value:''}).value;
  var ref=(document.getElementById('sl-ref')||{value:''}).value.trim();
  if(!date){toast('Bill date required','warning');return;}

  var selectedItems=[];
  document.querySelectorAll('.sl-item-chk:checked').forEach(function(chk){
    var itemId=chk.getAttribute('data-item-id');
    var rate=parseFloat(chk.getAttribute('data-rate'))||0;
    var qtyInp=document.querySelector('.sl-qty-inp[data-item-id="'+itemId+'"]');
    ...
    selectedItems.push({
      ...
      item_name:item.short_name||item.description||'',
      unit:item.unit||'',
      boq_qty:parseFloat(item.boq_qty)||0,
      billed_qty:qty,
      rate:rate,
  ...
  var additions=[];
  document.querySelectorAll('.bl-add-row').forEach(function(row){
    var head=(row.querySelector('.bl-add-head')||{value:''}).value.trim()||'Addition';
    var type=row.getAttribute('data-add-type')||'flat';
    var pct=parseFloat(row.getAttribute('data-add-pct'))||0;
    var amt=parseFloat((row.querySelector('.bl-add-amt')||{value:0}).value)||0;
    if(amt>0) additions.push({head:head,type:type,pct:pct,amount:amt});
  });
  // Collect deductions
  var deductions=[];
  document.querySelectorAll('#bl-ded-list > div').forEach(function(row){
    ...
    if(head&&amt>0) deductions.push({head:head,amount:amt});
  });
  // Collect GST
  document.querySelectorAll('.bl-gst-row').forEach(function(row){
  ...
  var renderGroup=function(numMap,label,col,docPrefix){
    ...
    return '<div style="margin-bottom:16px;">'+
      ...
      keys.map(function(num){
        ...
        return '<div style="background:var(--card-bg);border-radius:12px;border:1px solid var(--border);margin-bottom:8px;padding:12px 14px;display:flex;align-items:center;gap:10px;">'+
...
function execRenderAllotted(){
  ...
  el.innerHTML = '<div style="display:flex;justify-content:flex-end;gap:8px;margin-bottom:10px;"><button onclick="execAllottedDownloadExcel()" style="background:#2E7D32;color:white;border:none;border-radius:8px;padding:7px 12px;font-size:11px;font-weight:800;cursor:pointer;">&#128202; Excel</button><button onclick="execAllottedDownloadPDF()" style="background:#C62828;color:white;border:none;border-radius:8px;padding:7px 12px;font-size:11px;font-weight:800;cursor:pointer;">&#128196; PDF</button></div>'+
    ...
    return '<div style="background:var(--card-bg);border-radius:14px;border:1px solid var(--border);margin-bottom:12px;overflow:hidden;">'+
...
async function execGenBatchDoc(batchKey, docType){
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8">'+
    ...
    '<style>'+
      ...
      'body{font-family:"Arial",sans-serif;font-size:12px;color:#1a1a1a;background:var(--card-bg);}'+
...
function execDailyItemSummaries(rows){
  var byItem={},order=[];
  rows.forEach(function(row){
    var key=row.item.id||row.item.item_code||'unknown';
    if(!byItem[key]){
      byItem[key]={item:row.item,seenEntries:{},totalQtyDone:0,unit:row.item.unit||'',resources:{},resOrder:[],itemTotalAmount:0};
      order.push(key);
    }
    var g=byItem[key];
    var entryKey=row.d.id||row.d;
    if(!g.seenEntries[entryKey]){
      g.seenEntries[entryKey]=true;
      g.totalQtyDone+=parseFloat(row.d.qty_done)||0;
      if(!g.unit) g.unit=row.d.unit||row.item.unit||'';
    }
    if(row.res){
      var rkey=row.res.type+'||'+row.res.party+'||'+(row.res.unit||'');
      if(!g.resources[rkey]){g.resources[rkey]={type:row.res.type,party:row.res.party,unit:row.res.unit||'',totalQty:0,totalAmount:0};g.resOrder.push(rkey);}
      g.resources[rkey].totalQty+=parseFloat(row.res.qty)||0;
      g.resources[rkey].totalAmount+=row.res.amount;
      g.itemTotalAmount+=row.res.amount;
    }
  });
  return order.map(function(k){
    var g=byItem[k];
    return {item:g.item,totalQtyDone:g.totalQtyDone,unit:g.unit,resources:g.resOrder.map(function(rk){return g.resources[rk];}),itemTotalAmount:g.itemTotalAmount};
  });
}
...
function execDailyDownloadPDF(){
  ...
  var itemSummaries=execDailyItemSummaries(rows);
  var grandTotal2=0;
...
var SALES_SUBTAB='genbill'; // genbill | viewbills
...
function execRenderBills(){
  ...
  if(BILL_SUBTAB==='summary'){
    ...
    var grandGross=0,grandNet=0,grandPaid=0,grandBal=0;
    var summaryRows=Object.keys(partyMap).map(function(k){
      ...
      grandNet=(grandNet||0)+netPayable;
    ...
    var totalRow='<div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:0;border:2px solid #1A237E;border-radius:10px;margin-top:4px;overflow:hidden;">'+
      ...
      '<div style="padding:10px 14px;background:#1A237E;text-align:right;">'+
        ...
        '<div style="font-size:9px;color:rgba(255,255,255,0.6);margin-top:2px;">Net: '+inr(grandNet)+'</div>'+
      '</div>'+
      '<div style="padding:10px 14px;background:#1A237E;text-align:right;">'+
  ...
  var html=keys.map(function(key){
    ...
    var summaryHtml=
      '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:8px;">'+
        '<div style="text-align:center;background:var(--card-bg);padding:6px;border-radius:6px;"><div style="font-size:9px;color:var(--text3);">Billed Amount</div><div style="font-size:12px;font-weight:900;color:#1A237E;">'+inr(totalBilledAmt)+'</div></div>'+
        '<div style="text-align:center;background:var(--card-bg);padding:6px;border-radius:6px;"><div style="font-size:9px;color:var(--text3);">Net Payable</div><div style="font-size:12px;font-weight:900;color:#1565C0;">'+inr(netPayableAmt)+'</div></div>'+
        '<div style="text-align:center;background:var(--card-bg);padding:6px;border-radius:6px;"><div style="font-size:9px;color:var(--text3);">Total Paid</div><div style="font-size:12px;font-weight:900;color:#2E7D32;">'+inr(totalPaid)+'</div></div>'+
        '<div style="text-align:center;background:var(--card-bg);padding:6px;border-radius:6px;"><div style="font-size:9px;color:'+(balanceDue>0?'#C62828':'#2E7D32')+';">Balance Due</div><div style="font-size:12px;font-weight:900;color:'+(balanceDue>0?'#C62828':'#2E7D32')+';">'+inr(Math.abs(balanceDue))+(balanceDue<0?' Cr':'')+'</div></div>'+
      '</div>'+
      '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin-bottom:8px;">'+
        '<div style="text-align:center;background:var(--card-bg);padding:6px;border-radius:6px;"><div style="font-size:9px;color:var(--text3);">Advances Paid</div><div style="font-size:12px;font-weight:900;color:#F57F17;">'+inr(totalAdv)+'</div></div>'+
        '<div style="text-align:center;background:var(--card-bg);padding:6px;border-radius:6px;"><div style="font-size:9px;color:var(--text3);">Cash/Bank Paid</div><div style="font-size:12px;font-weight:900;color:#2E7D32;">'+inr(totalCash)+'</div></div>'+
      '</div>';
...
function grnRender(){
  ...
  var summaryBar=
    '<div style="display:flex;gap:8px;margin-bottom:12px;">'+
      '<div style="background:var(--card-bg);border-radius:10px;padding:8px 14px;border-left:4px solid #558B2F;flex:1;">'+
      ...
      '<div style="background:var(--card-bg);border-radius:10px;padding:8px 14px;border-left:4px solid #E65100;flex:1;">'+
  ...
  el.innerHTML=
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px;">'+
      ...
      '<div style="display:flex;gap:8px;">'+
        '<button onclick="grnDownloadAllExcel()" style="background:#2E7D32;color:white;border:none;border-radius:8px;padding:6px 10px;font-size:11px;font-weight:800;cursor:pointer;">&#128202; Excel</button>'+
        '<button onclick="grnDownloadAllPDF()" style="background:#C62828;color:white;border:none;border-radius:8px;padding:6px 10px;font-size:11px;font-weight:800;cursor:pointer;">&#128196; PDF</button>'+
      '</div>'+
    '</div>'+
    summaryBar+pendingSection+
    (grnList||(!pending.length?'<div style="text-align:center;padding:30px;color:var(--text3);background:var(--card-bg);border-radius:12px;">No vendor allotments found for this project</div>':''));
}
function grnDownloadAllExcel(){
  ...
  var lines=[[compName],['GRN Statement - '+projName],[''],header];
  GRN_ITEMS.forEach(function(g){
    var allot=GRN_ALLOTS.find(function(a){return a.id===g.allot_id;})||{};
    var planRes=WA_PLANNED.find(function(p){return p.id===allot.boq_exec_resource_id;})||{};
    var resName=planRes.party_name||planRes.resource_category||'';
    var boqItem=WA_ITEMS.find(function(i){return i.id===allot.boq_item_id;})||{};
    lines.push([g.grn_number,fmtD2(g.grn_date),resName,allot.party_name||'',(boqItem.item_code?'['+boqItem.item_code+'] ':'')+(boqItem.short_name||boqItem.description||''),allot.qty||'',g.qty_received,g.unit||allot.unit||'',stLbl[g.status]||g.status,g.approval_status==='approved'?'Approved':'Pending',g.created_by||'',g.remarks||'']);
  });
  var csv=lines.map(function(row){
    return row.map(function(cell){
      var s=String(cell==null?'':cell);
  ...
  var a=document.createElement('a');
  a.href=URL.createObjectURL(blob);
  a.download='GRN_'+(projName||'Project').replace(/[^a-z0-9]/gi,'_')+'.csv';
  a.click();
  toast('GRN Excel downloaded','success');
}
function grnDownloadAllPDF(){
  ...
  var today=typeof fmtDate==='function'?fmtDate(new Date()):new Date().toLocaleDateString();
  var stLbl={accepted:'Accepted',rejected:'Rejected',partial:'Partial'};
  var stCol={accepted:'#2E7D32',rejected:'#C62828',partial:'#F57F17'};
  function fmtD2(d){if(!d)return '—';if(/^\d{4}-\d{2}-\d{2}/.test(d)){var p=d.split('-');return p[2]+'/'+p[1]+'/'+p[0];}return d;}
  var thStyle='padding:6px 8px;font-size:9px;font-weight:800;text-align:right;white-space:nowrap;';
  var thead='<tr style="background:#558B2F;color:white;">'+
    '<th style="'+thStyle+'text-align:left;">GRN Number</th>'+
    '<th style="'+thStyle+'text-align:center;">Date</th>'+
    '<th style="'+thStyle+'text-align:left;">Resource</th>'+
    '<th style="'+thStyle+'text-align:left;">Party</th>'+
    '<th style="'+thStyle+'text-align:left;">BOQ Item</th>'+
    '<th style="'+thStyle+'">Ordered</th>'+
    '<th style="'+thStyle+'background:#2E7D32;">Received</th>'+
    '<th style="'+thStyle+'text-align:center;">Status</th>'+
    '<th style="'+thStyle+'text-align:center;">Approval</th>'+
    '<th style="'+thStyle+'text-align:left;">Created By</th>'+
    '<th style="'+thStyle+'text-align:left;">Remarks</th>'+
  '</tr>';
  var tbody='';
  GRN_ITEMS.forEach(function(g,i){
    var allot=GRN_ALLOTS.find(function(a){return a.id===g.allot_id;})||{};
    var planRes=WA_PLANNED.find(function(p){return p.id===allot.boq_exec_resource_id;})||{};
    var resName=planRes.party_name||planRes.resource_category||'';
    var boqItem=WA_ITEMS.find(function(x){return x.id===allot.boq_item_id;})||{};
    ...
    tbody+='<tr style="border-bottom:1px solid #F0F0F0;background:'+bg+'">'+
      ...
      '<td style="padding:5px 8px;font-size:9px;">'+resName+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;">'+(allot.party_name||'')+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;">'+(boqItem.item_code?'['+boqItem.item_code+'] ':'')+(boqItem.short_name||boqItem.description||'')+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;text-align:right;">'+(allot.qty||'')+' '+(allot.unit||'')+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;text-align:right;font-weight:700;color:#2E7D32;">'+g.qty_received+' '+(g.unit||allot.unit||'')+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;text-align:center;font-weight:700;color:'+sc+';">'+(stLbl[g.status]||g.status)+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;text-align:center;">'+(g.approval_status==='approved'?'Approved':'Pending')+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;">'+(g.created_by||'')+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;">'+(g.remarks||'')+'</td>'+
    '</tr>';
  });

  var html='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>GRN Statement - '+projName+'</title>'+
    '<style>'+
    '*{margin:0;padding:0;box-sizing:border-box;}'+
    'body{font-family:Arial,sans-serif;font-size:10px;color:#222;padding:14px;}'+
    '.hdr{text-align:center;border-bottom:3px double #558B2F;padding-bottom:8px;margin-bottom:10px;}'+
    '.logo{font-size:16px;font-weight:900;color:#558B2F;}.sub{font-size:9px;color:#555;margin-top:2px;}'+
    'table{width:100%;border-collapse:collapse;}'+
    '.meta{display:flex;justify-content:space-between;margin-bottom:8px;font-size:9px;}'+
    '@media print{'+
      'button{display:none;}'+
    ...
    '}'+
    '</style></head><body>'+
    '<button onclick="window.print()" style="margin-bottom:10px;padding:6px 16px;background:#558B2F;color:white;border:none;border-radius:6px;cursor:pointer;font-family:Arial;font-weight:700;font-size:11px;">Print / Save PDF</button>'+
    '<div class="hdr">'+
      '<div class="logo">'+compName+'</div>'+
      (compAddr?'<div class="sub">'+compAddr+'</div>':'')+
      (compGST?'<div class="sub">GSTIN: '+compGST+'</div>':'')+
    '</div>'+
    '<div class="meta">'+
      '<b>GRN Statement &mdash; '+projName+'</b>'+
      '<span>Generated: '+today+'</span>'+
    '</div>'+
    '<table><thead>'+thead+'</thead><tbody>'+tbody+'</tbody></table>'+
    '<p style="font-size:8px;color:#888;text-align:center;margin-top:8px;border-top:1px solid #eee;padding-top:4px;">'+
      compName+' | GRN Statement | '+projName+' | Generated on '+today+
    '</p>'+
    '<script>window.onload=function(){window.print();}<\/script>'+
    '</body></html>';
...
function grnOpenForm(allotId, projId){
  ...
  if(!shTitle||!shBody||!shFoot){
    ...
    if(!document.getElementById('grn-sheet-overlay')){
      ...
      sh.id='grn-sheet';
      sh.style.cssText='position:fixed;bottom:0;left:0;right:0;background:var(--card-bg);border-radius:20px 20px 0 0;z-index:9999;display:none;max-height:85vh;overflow-y:auto;';
...
function storeRender(){
  ...
  if(!STORE_ITEMS.length){
    el.innerHTML=
      '<div style="text-align:center;padding:40px;color:var(--text3);background:var(--card-bg);border-radius:12px;">'+
  ...
  var summaryBar=
    '<div style="display:flex;gap:8px;margin-bottom:12px;">'+
      '<div style="background:var(--card-bg);border-radius:10px;padding:8px 14px;border-left:4px solid #6A1B9A;flex:1;">'+
      ...
      (lowStock>0?'<div style="background:var(--card-bg);border-radius:10px;padding:8px 14px;border-left:4px solid #C62828;flex:1;">'+
  ...
  el.innerHTML=
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px;">'+
      ...
      '<div style="display:flex;gap:8px;">'+
        '<button onclick="storeDownloadExcel()" style="background:#2E7D32;color:white;border:none;border-radius:8px;padding:6px 10px;font-size:11px;font-weight:800;cursor:pointer;">&#128202; Excel</button>'+
        '<button onclick="storeDownloadPDF()" style="background:#C62828;color:white;border:none;border-radius:8px;padding:6px 10px;font-size:11px;font-weight:800;cursor:pointer;">&#128196; PDF</button>'+
      '</div>'+
    '</div>'+
    summaryBar+
    '<div style="background:var(--card-bg);border-radius:14px;overflow:hidden;">'+
...
function storeGroupedItems(){
  var grouped={},groupOrder=[];
  STORE_ITEMS.forEach(function(item){
    var key=(item.item_name||'').toLowerCase()+'__'+(item.unit||'').toLowerCase();
    if(!grouped[key]){
      grouped[key]={item_name:item.item_name,unit:item.unit||'',qty_in_hand:0,qty_issued:0,min_qty:parseFloat(item.min_qty)||0,last_grn_date:item.last_grn_date||''};
      groupOrder.push(key);
    }
    grouped[key].qty_in_hand+=(parseFloat(item.qty_in_hand)||0);
    grouped[key].qty_issued+=(parseFloat(item.qty_issued)||0);
    if(item.last_grn_date&&item.last_grn_date>grouped[key].last_grn_date) grouped[key].last_grn_date=item.last_grn_date;
  });
  return groupOrder.map(function(k){return grouped[k];});
}
function storeDownloadExcel(){
  ...
  items.forEach(function(g){
    lines.push([g.item_name,g.qty_in_hand,g.qty_issued,g.qty_in_hand+g.qty_issued,g.unit,fmtD2(g.last_grn_date)]);
  });
  var projLogs=STORE_ISSUE_LOG.filter(function(l){return l.project_id===(STORE_PROJ_ID||'');});
  if(projLogs.length){
    lines.push([]);
    lines.push(['ISSUE LOG']);
    lines.push(['Material','Qty Issued','Unit','Issued To','Date','Status']);
    projLogs.forEach(function(log){
      lines.push([log.item_name||'',parseFloat(log.qty_issued)||0,log.unit||'',log.issued_to||'',fmtD2(log.issue_date),(log.status||'available')]);
    });
  }
  var csv=lines.map(function(row){
    return row.map(function(cell){
  ...
  var a=document.createElement('a');
  a.href=URL.createObjectURL(blob);
  a.download='Store_'+(projName||'Project').replace(/[^a-z0-9]/gi,'_')+'.csv';
  a.click();
  toast('Store Excel downloaded','success');
}
function storeDownloadPDF(){
  var items=storeGroupedItems();
  if(!items.length){toast('Store is empty, nothing to export','warning');return;}
  var projName=execProjName();
  var compName=typeof coName==='function'?coName():'RYDAX';
  ...
  function fmtD2(d){if(!d)return '—';if(/^\d{4}-\d{2}-\d{2}/.test(d)){var p=d.split('-');return p[2]+'/'+p[1]+'/'+p[0];}return d;}
  var thStyle='padding:6px 8px;font-size:9px;font-weight:800;text-align:right;white-space:nowrap;';
  var thead='<tr style="background:#6A1B9A;color:white;">'+
    '<th style="'+thStyle+'text-align:left;">Material</th>'+
    '<th style="'+thStyle+'background:#4A148C;">In Hand</th>'+
    '<th style="'+thStyle+'">Issued</th>'+
    '<th style="'+thStyle+'">Total In</th>'+
    '<th style="'+thStyle+'text-align:center;">Unit</th>'+
    '<th style="'+thStyle+'text-align:center;">Last GRN Date</th>'+
  '</tr>';
  var tbody='';
  items.forEach(function(g,i){
    ...
    tbody+='<tr style="border-bottom:1px solid #F0F0F0;background:'+bg+'">'+
      ...
      '<td style="padding:5px 8px;font-size:9px;text-align:right;color:#E65100;">'+g.qty_issued.toFixed(2)+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;text-align:right;">'+(g.qty_in_hand+g.qty_issued).toFixed(2)+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;text-align:center;">'+g.unit+'</td>'+
      '<td style="padding:5px 8px;font-size:9px;text-align:center;">'+fmtD2(g.last_grn_date)+'</td>'+
    '</tr>';
  });
  ...
  var logSection='';
  if(projLogs.length){
    var logThead='<tr style="background:#4A148C;color:white;">'+
      '<th style="padding:6px 8px;font-size:9px;font-weight:800;text-align:left;">Material</th>'+
      '<th style="padding:6px 8px;font-size:9px;font-weight:800;text-align:right;">Qty Issued</th>'+
      '<th style="padding:6px 8px;font-size:9px;font-weight:800;text-align:left;">Issued To</th>'+
      '<th style="padding:6px 8px;font-size:9px;font-weight:800;text-align:center;">Date</th>'+
    ...
    var logTbody=projLogs.map(function(log,i){
      ...
      return '<tr style="border-bottom:1px solid #F0F0F0;background:'+bg+'">'+
        ...
        '<td style="padding:5px 8px;font-size:9px;">'+(log.issued_to||'')+'</td>'+
        '<td style="padding:5px 8px;font-size:9px;text-align:center;">'+fmtD2(log.issue_date)+'</td>'+
        '<td style="padding:5px 8px;font-size:9px;text-align:center;">'+(log.status||'available')+'</td>'+
      '</tr>';
    }).join('');
    logSection='<div class="sec-title">Issue Log</div><table><thead>'+logThead+'</thead><tbody>'+logTbody+'</tbody></table>';
  }

  var html='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Store Inventory - '+projName+'</title>'+
    '<style>'+
    '*{margin:0;padding:0;box-sizing:border-box;}'+
    'body{font-family:Arial,sans-serif;font-size:10px;color:#222;padding:14px;}'+
    '.hdr{text-align:center;border-bottom:3px double #6A1B9A;padding-bottom:8px;margin-bottom:10px;}'+
    '.logo{font-size:16px;font-weight:900;color:#6A1B9A;}.sub{font-size:9px;color:#555;margin-top:2px;}'+
    'table{width:100%;border-collapse:collapse;margin-bottom:16px;}'+
    '.meta{display:flex;justify-content:space-between;margin-bottom:8px;font-size:9px;}'+
    '.sec-title{font-size:11px;font-weight:800;color:#6A1B9A;margin-bottom:6px;}'+
    '@media print{'+
      'button{display:none;}'+
      '@page{size:A4 landscape;margin:8mm;}'+
      'body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}'+
    '}'+
    '</style></head><body>'+
    '<button onclick="window.print()" style="margin-bottom:10px;padding:6px 16px;background:#6A1B9A;color:white;border:none;border-radius:6px;cursor:pointer;font-family:Arial;font-weight:700;font-size:11px;">Print / Save PDF</button>'+
    '<div class="hdr">'+
      '<div class="logo">'+compName+'</div>'+
      (compAddr?'<div class="sub">'+compAddr+'</div>':'')+
      (compGST?'<div class="sub">GSTIN: '+compGST+'</div>':'')+
    '</div>'+
    '<div class="meta">'+
      '<b>Store Inventory &mdash; '+projName+'</b>'+
      '<span>Generated: '+today+'</span>'+
    '</div>'+
    '<table><thead>'+thead+'</thead><tbody>'+tbody+'</tbody></table>'+
    logSection+
    '<p style="font-size:8px;color:#888;text-align:center;margin-top:8px;border-top:1px solid #eee;padding-top:4px;">'+
      compName+' | Store Inventory | '+projName+' | Generated on '+today+
    '</p>'+
    '<script>window.onload=function(){window.print();}<\/script>'+
    '</body></html>';
