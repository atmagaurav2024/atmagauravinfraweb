...
async function boqLoadItems(){
  try{
    ...
    var data=await sbFetch('boq_items',{select:'*',filter:filter,order:'sort_order.asc'});
...
function boqRender(){
  ...
  html+='<div class="table-card"><table class="att-table"><thead><tr><th>#</th><th>Description</th><th>Unit</th><th>Qty</th><th>Rate</th><th>Amount</th><th>Actions</th></tr></thead><tbody>'+
    BOQ_ITEMS.map(function(item,i){
      ...
      return '<tr><td>'+item.item_no+'</td><td>'+item.description+'<br><small style="color:var(--text3);">'+item.specification+'</small></td><td>'+item.unit+'</td><td>'+item.qty+'</td><td>'+fmtINR(item.rate)+'</td><td><strong>'+fmtINR(amt)+'</strong></td>'+
        '<td><button class="btn btn-sm btn-navy" onclick="boqEditItem(\''+item.id+'\')">✏</button> <button class="btn btn-sm btn-red" onclick="boqDeleteItem(\''+item.id+'\')">🗑</button></td></tr>';
