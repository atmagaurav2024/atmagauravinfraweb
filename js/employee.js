...
async function initEmpMgmt(){
  ...
  try{
    ...
    var needLeaveFix = !EMP_LEAVEFIX.length;

    if(needEmps || needPay || needLeaveFix){
      var fetches = [
        needEmps
        ...
        needPay
          ...
          : Promise.resolve(null),
        needLeaveFix
          ? sbFetch('employee_leave_fixation',{select:'*'}).catch(function(e){ console.error('employee_leave_fixation fetch failed — table may not exist yet:',e); return []; })
          : Promise.resolve(null)
      ];
      var r = await Promise.all(fetches);
      if(r[0]!==null) EMP_LIST = Array.isArray(r[0]) ? r[0] : [];
      if(r[1]!==null) EMP_PAY  = Array.isArray(r[1]) ? r[1] : [];
      if(r[2]!==null) EMP_LEAVEFIX = Array.isArray(r[2]) ? r[2] : [];
    }
  ...
  var allTabs = ['active','pending','pay','leavefix','salary','increment','transfer','annual','downloads','resigned'];
...
// ── LEAVE FIXATION ────────────────────────────────────
// Fixes, per employee, how many leaves of each leave type (leave types
// come from Master Registry / CAT_DATA.leave, not re-entered here) they
// are entitled to and over what period (per Year / per Month / per
// Quarter). Stored in employee_leave_fixation, one row per employee per
// leave type.
...
function empLeaveFixHTML(){
  ...
  return active.map(function(e){
    var name=(e.first_name||'')+(e.middle_name?' '+e.middle_name:'')+(e.last_name?' '+e.last_name:'');
    var rows=EMP_LEAVEFIX.filter(function(f){return f.employee_id===e.id;});

    var chipsHTML = rows.length
      ? '<div style="display:flex;flex-wrap:wrap;gap:6px;padding:0 14px 12px;">'+
          rows.map(function(f){
            return '<span style="background:#E8F5E9;color:#2E7D32;font-size:11px;font-weight:700;padding:4px 10px;border-radius:20px;">'+
              (f.leave_type||'Leave')+': '+(f.quota||0)+' '+(LEAVEFIX_PERIOD_LABEL[f.period]||f.period||'')+
            '</span>';
          }).join('')+
        '</div>'
    ...
    return '<div style="background:white;border-radius:12px;margin-bottom:10px;box-shadow:0 1px 3px rgba(0,0,0,.08);overflow:hidden;">'+
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;">'+
        '<div>'+
          ...
          '<div style="font-size:10.5px;color:var(--text3);">'+(e.designation||e.role||'')+'</div>'+
        '</div>'+
        '<button onclick="empOpenLeaveFix(\''+e.id+'\')" style="background:#2E7D32;color:white;border:none;border-radius:8px;padding:7px 14px;font-size:11.5px;font-weight:800;cursor:pointer;">&#127796; Fix Leaves</button>'+
      '</div>'+
      chipsHTML+
    '</div>';
  }).join('');
...
function empOpenLeaveFix(empId){
  var e=EMP_LIST.find(function(x){return x.id===empId;});
  if(!e){toast('Employee not found','warning');return;}
  var name=(e.first_name||'')+(e.middle_name?' '+e.middle_name:'')+(e.last_name?' '+e.last_name:'');
  var leaveTypes=(CAT_DATA.leave||[]).filter(function(c){return c.active;});
  var existing={};
  EMP_LEAVEFIX.filter(function(f){return f.employee_id===empId;}).forEach(function(f){existing[f.leave_type]=f;});

  document.getElementById('leavefix-sheet-title').textContent='\u{1F334} Leave Fixation \u2014 '+name;
  document.getElementById('leavefix-sheet-body').innerHTML=
    '<div style="font-size:11px;color:var(--text3);margin-bottom:14px;">Set the leave quota and period for each leave type. Leave a quota blank/0 to skip that type. Leave types themselves are managed under Settings &rarr; Master Registry.</div>'+
    (leaveTypes.length ? leaveTypes.map(function(lt){
      var ex=existing[lt.name];
      return '<div style="display:grid;grid-template-columns:1.4fr 0.9fr 1fr;gap:8px;align-items:center;padding:8px 0;border-bottom:1px solid #F0F0F0;">'+
        '<div style="font-size:12.5px;font-weight:700;">'+lt.icon+' '+lt.name+'</div>'+
        '<input type="number" min="0" step="0.5" id="lf-qty-'+lt.id+'" class="finp" placeholder="Qty" value="'+(ex?ex.quota:'')+'" style="margin:0;padding:7px 8px;">'+
        '<select id="lf-period-'+lt.id+'" class="fsel" style="margin:0;padding:7px 8px;">'+
          '<option value="year"'+(!ex||ex.period==='year'?' selected':'')+'>Per Year</option>'+
          '<option value="month"'+(ex&&ex.period==='month'?' selected':'')+'>Per Month</option>'+
          '<option value="quarter"'+(ex&&ex.period==='quarter'?' selected':'')+'>Per Quarter</option>'+
        '</select>'+
      '</div>';
    }).join('') : '<div style="text-align:center;padding:20px;color:var(--text3);font-size:12px;">No leave types found in Master Registry. Add leave types there first.</div>');

  var foot=document.getElementById('leavefix-sheet-foot');
  foot.innerHTML='';
  var cb=document.createElement('button');cb.className='btn btn-outline';cb.textContent='Cancel';
  cb.onclick=function(){closeSheet('ov-leavefix','sh-leavefix');};
  var sb=document.createElement('button');sb.className='btn btn-navy';sb.textContent='Save';
  sb.onclick=function(){empSaveLeaveFix(empId,leaveTypes);};
  foot.appendChild(cb);foot.appendChild(sb);
  openSheet('ov-leavefix','sh-leavefix');
}

async function empSaveLeaveFix(empId,leaveTypes){
  var rows=[];
  leaveTypes.forEach(function(lt){
    var qtyInp=document.getElementById('lf-qty-'+lt.id);
    var periodInp=document.getElementById('lf-period-'+lt.id);
    var qty=qtyInp?parseFloat(qtyInp.value):0;
    if(qty>0){
      rows.push({leave_type:lt.name, quota:qty, period:periodInp?periodInp.value:'year'});
    }
  });
  try{
    // Replace this employee's leave fixation rows wholesale — simplest way
    // to keep it consistent with whatever is currently on the form
    // (including types that were cleared back to 0/blank).
    var toDelete=EMP_LEAVEFIX.filter(function(f){return f.employee_id===empId;});
    for(var i=0;i<toDelete.length;i++){ await sbDelete('employee_leave_fixation',toDelete[i].id); }
    EMP_LEAVEFIX=EMP_LEAVEFIX.filter(function(f){return f.employee_id!==empId;});

    for(var j=0;j<rows.length;j++){
      var res=await sbInsert('employee_leave_fixation',{employee_id:empId,leave_type:rows[j].leave_type,quota:rows[j].quota,period:rows[j].period,effective_date:new Date().toISOString().slice(0,10)});
      if(res&&res[0]) EMP_LEAVEFIX.push(res[0]);
    }
    closeSheet('ov-leavefix','sh-leavefix');
    toast('Leave quotas saved','success');
    empRender();
  }catch(e){ toast('Error: '+e.message,'error'); }
}
